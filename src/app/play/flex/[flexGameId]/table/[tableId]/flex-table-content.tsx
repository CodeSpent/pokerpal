...
import { Wifi, WifiOff, LogOut, Plus, Clock, FileText, ScrollText } from 'lucide-react';
...
export default function FlexTableContent({
  ...
  const { flexGameId, tableId } = use(params);
  ...
  const [gameInfo, setGameInfo] = useState<{ minBuyIn: number; maxBuyIn: number; status: string; turnTimerHours: number } | null>(null);
  ...
  // Subscribe to flex game channel for close events
  ...
  useEffect(() => {
    fetch(`/api/flex-games/${flexGameId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.flexGame) {
          setGameInfo({
            minBuyIn: data.flexGame.minBuyIn,
            maxBuyIn: data.flexGame.maxBuyIn,
            status: data.flexGame.status,
            turnTimerHours: data.flexGame.turnTimerHours,
          });
          setRebuyAmount(data.flexGame.maxBuyIn);
          if (data.flexGame.status === 'closed' || data.flexGame.status === 'expired') {
            setGameClosed(true);
          }
        }
      })
  ...
  // Client-side timeout trigger (flex timers are long, but still trigger on expiry)
  useEffect(() => {
  ...
  const isHeroTurn = heroSeatIndex !== null && currentActorSeatIndex === heroSeatIndex;
  ...
  const handleLeave = async () => {
    setIsLeaving(true);
    try {
      const res = await fetch(`/api/flex-games/${flexGameId}/leave`, { method: 'POST' });
  ...
  const handleRebuy = async () => {
    setIsRebuying(true);
    try {
      const res = await fetch(`/api/flex-games/${flexGameId}/rebuy`, {
        method: 'POST',
  ...
  // Format remaining time for flex timer
  const formatFlexTimer = (expiresAt: number | null | undefined) => {
    ...
    return (
      <div className="h-[calc(100dvh-11.5rem)] flex items-center justify-center bg-surface-primary">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push('/play?tab=flex')}
    ...
    <div className="h-[calc(100dvh-11.5rem)] flex flex-col bg-surface-primary overflow-hidden">
      ...
      <header className="h-12 shrink-0 flex items-center justify-between px-4 bg-surface-secondary border-b border-surface-tertiary">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-purple-400 font-medium">Flex Game</span>

          {/* Turn indicator */}
          {isHeroTurn && (
            <>
              <div className="w-px h-4 bg-surface-tertiary" />
              <span className="text-xs text-emerald-400 font-bold animate-pulse">Your Turn!</span>
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Flex timer (hours/minutes) */}
          {heroSeatIndex !== null && turnExpiresAt && (
            <div className="flex items-center gap-1 text-xs text-purple-400">
              <Clock className="w-3 h-3" />
              <span className="font-mono">{formatFlexTimer(turnExpiresAt)}</span>
            </div>
          )}

          {/* Standard turn timer as fallback */}
          {heroSeatIndex !== null && !turnExpiresAt && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-zinc-900 rounded-xl p-8 border border-zinc-700 text-center max-w-sm mx-4">
            ...
            <p className="text-zinc-400 mb-6">
              The host has closed this flex game. Your chips have been returned to your balance.
            </p>
            <button
              onClick={() => router.push('/play?tab=flex')}
