...
import { ArrowLeft, Users, Coins, DollarSign, LogOut, X, Clock } from 'lucide-react';
import { ShareButton } from '@/components/ui/share-button';

interface FlexGameDetails {
  id: string;
  name: string;
  status: string;
  maxPlayers: number;
...
export default function FlexGameContent({
  params,
}: {
  params: Promise<{ flexGameId: string }>;
}) {
  const { flexGameId } = use(params);
  const router = useRouter();
  const { data: session } = useSession();
  const playerId = session?.user?.playerId ?? null;

  const [game, setGame] = useState<FlexGameDetails | null>(null);
  ...
  const flexGameChannel = useChannel(`flex-game-${flexGameId}`);
  ...
  useChannelEvent(flexGameChannel, 'PLAYER_JOINED', handlePlayerJoined);
  useChannelEvent(flexGameChannel, 'PLAYER_LEFT', handlePlayerLeft);
  useChannelEvent(flexGameChannel, 'FLEX_GAME_CLOSED', handleGameClosed);

  const fetchGameDetails = async () => {
    try {
      const res = await fetch(`/api/flex-games/${flexGameId}`);
      const data = await res.json();
      if (data.flexGame) {
        setGame(data.flexGame);
        setTableId(data.table?.id ?? null);
        setPlayers(data.players || []);
        if (buyInAmount === 0 && data.flexGame.minBuyIn) {
          setBuyInAmount(data.flexGame.maxBuyIn);
        }
      }
    } catch (err) {
      console.error('Failed to fetch flex game:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleJoin = async () => {
    ...
    try {
      const res = await fetch(`/api/flex-games/${flexGameId}/join`, {
        method: 'POST',
      ...
      if (data.success && data.tableId) {
        router.push(`/play/flex/${flexGameId}/table/${data.tableId}`);
      } else {
        setError(data.error || 'Failed to join');
      }
    } catch {
      setError('Failed to join flex game');
    } finally {
      setIsJoining(false);
    }
  };
  ...
  const handleClose = async () => {
    ...
    try {
      const res = await fetch(`/api/flex-games/${flexGameId}/close`, {
        method: 'POST',
      });

      const data = await res.json();
      if (data.success) {
        router.push('/play?tab=flex');
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-zinc-400">Loading flex game...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/play?tab=flex" className="text-emerald-400 hover:underline">
            Back to Lobby
          </Link>
        </div>
      </div>
  ...
  if (game.status === 'closed' || game.status === 'expired') {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <X className="w-12 h-12 mx-auto text-zinc-600 mb-4" />
          <h2 className="text-xl font-bold text-white mb-2">
            {game.status === 'expired' ? 'Game Expired' : 'Game Closed'}
          </h2>
          <p className="text-zinc-400 mb-4">
            {game.status === 'expired'
              ? 'This flex game expired due to inactivity.'
              : 'This flex game has been closed by the host.'}
          </p>
          <Link
            href="/play?tab=flex"
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex-1">
            ...
            <p className="text-zinc-400">
              {game.smallBlind}/{game.bigBlind} Flex Game
            </p>
          </div>
          <ShareButton
            variant="compact"
            url={`https://letsplay.poker/play/flex/${flexGameId}`}
            title={game.name}
            text={`Join my ${game.smallBlind}/${game.bigBlind} flex game on LetsPlay Poker! ${game.turnTimerHours}h per turn.`}
          />
        </div>
        ...
        >
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            ...
            <div>
              <div className="text-xs text-zinc-500 mb-1">Turn Timer</div>
              <div className="flex items-center gap-1 text-white">
                <Clock className="w-4 h-4 text-purple-400" />
                <span className="font-mono">{game.turnTimerHours}h</span>
              </div>
            </div>
            <div>
          ...
          <div className="mb-6 p-3 rounded-lg bg-purple-500/10 border border-purple-500/20">
            <p className="text-sm text-purple-300">
              Flex game — take turns at your own pace ({game.turnTimerHours}h per turn). Fixed blinds, rebuy anytime.
            </p>
          </div>
